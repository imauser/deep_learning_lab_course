# Author note: this was mainly imported from Anders Boesen's nice convnet implementation
#              https://github.com/andersbll/nnet
#              to provide an alternative to the im2col based convolutions
...
def forward_pool(np.ndarray[DTYPE_t, ndim=4] imgs,
...
@cython.boundscheck(False)
@cython.wraparound(False)
def bprop_pool(np.ndarray[DTYPE_t, ndim=4] poolout_grad,
               np.ndarray[np.int_t, ndim=5] switches,
