...
class Augmentator():
        ...
        for i in images:
            for j in range(zooms):
                out.append(Augmentator.augment_keeping_image_size(Augmentator.zoom, i,\
                        zoom=[1+random.random(), 1+random.random(), 1]))
            for j in range(eds):
    ...
    @staticmethod
    def zoom(image, zoom=[1.5,1.5,1], *args, **kwargs):
    ...
    def augment_keeping_image_size(func, image, *args, **kwargs):
        ...
        offset = (new_image.shape[0] - shape[0])/2, (new_image.shape[0] - shape[0])/2
    ...
    @staticmethod
    def elastic_deformation(image, sigma=1.,alpha=100., random_state=None):
        """
